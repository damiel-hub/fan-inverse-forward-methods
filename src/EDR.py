...
class EDR: # Elevation-Distance Relationship
    ...
    def __init__(self, sMap, zMap, drawFlag = False):
        """
        Initializes the object with sMap and zMap data.
        """
        # Flatten the input matrices
        self.s_flatten_nan = sMap.flatten()
        self.z_flatten_nan = zMap.flatten()
        
        self.nan_index = np.isnan(self.s_flatten_nan) | np.isnan(self.z_flatten_nan)
 
        self.s_flatten = self.s_flatten_nan[~self.nan_index]
        self.z_flatten = self.z_flatten_nan[~self.nan_index]
    ...
    def medianFilter_on(self, bin_size, ds, outlength, pltFlag=False, drawFlag='b', return_fitting_coeff=False, return_s_width=False, mask_2d=None, bin_num = None):
            """
            ...
            Args:
                ...
                return_s_width (bool, optional): If True, returns the width of each s bin.
                mask_2d (np.ndarray, optional): A 2D boolean mask with the same shape as sMap and zMap. 
                                                If provided, RMSE will be calculated only within this mask.
            ...
            if bin_num is not None:
                bin_pts_s = np.linspace(0, np.max(self.s_flatten), bin_num + 1)
            else:
            ...
            # Remove NaN values that might arise from empty bins
            valid = ~np.isnan(Q2)
            bin_pts_mid_s = bin_pts_mid_s[valid]
            Q2 = np.array(Q2)[valid]
            width = np.array(width)[valid]
            ...
            if len(bin_pts_mid_s) > 0:
                ...
                if pltFlag:
                    # Plot the data
                    plt.figure()
                    plt.scatter(self.s_flatten, self.z_flatten, color='k', marker='.', s=0.2, alpha=0.5)
                ...
                s_width = np.column_stack((bin_pts_mid_s, width))

                if return_fitting_coeff:
                    # Calculate predicted elevation (z_hat) for all original data points
                    ...
                    # --- MODIFICATION START: RMSE CALCULATION ---
                    if mask_2d is not None:
                        # Flatten the mask and apply the same NaN filter as the data
                        mask_1d = mask_2d.flatten()[~self.nan_index].astype(bool)
                        
                        # Ensure there are points within the mask to avoid division by zero
                        if np.sum(mask_1d) > 0:
                            # Select only the points within the mask for RMSE calculation
                            z_true_masked = self.z_flatten[mask_1d]
                            z_hat_masked = z_hat[mask_1d]
                            RMSE_value = np.sqrt(np.sum((z_hat_masked - z_true_masked)**2) / z_true_masked.size)
                        else:
                            # If mask is empty, RMSE is not well-defined. Return NaN.
                            RMSE_value = np.nan
                    else:
                        # Original calculation if no mask is provided
                    ...
                    return fitting_s_z
            else: # Handle case where there are no valid bins
                # Return empty or appropriate default values
                if return_fitting_coeff:
                    return np.empty((0,2)), np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, s_T, np.empty(0), np.empty(0)
                elif return_s_width:
                    return np.empty((0,2))
                else:
                    return np.empty((0,2))
